...
> 参数解释
>
> 1）M：活跃地址的代币储蓄量平均占比，用于反映代币的储蓄价值。（注：表中该列数据为扩大10万倍后的数据）
>
> 2）R：活跃地址对代币的出入度指标之和，用于反映代币的流通价值。（注：表中该列数据为扩大10万倍后的数据）
>
> 3）N：活跃地址的代币流通次数。指在统计期间内，代币流入活跃地址的次数和从活跃地址流出的次数之和（即一次成功的交易会产生两次流通次数）
>
